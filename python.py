...
# Resend Email API Configuration for Direct Inquiries
RESEND_API_KEY = os.getenv("RESEND_API_KEY", "")
ADMIN_INQUIRY_EMAIL = os.getenv("ADMIN_INQUIRY_EMAIL", "adiityakumar9523340408@gmail.com")
RESEND_FROM_EMAIL = os.getenv("RESEND_FROM_EMAIL", "MediCare Hospital <onboarding@resend.dev>")
...
# ============================================
# DIRECT INQUIRY & EMAIL ENDPOINTS (RESEND API)
# ============================================

async def send_inquiry_email(inquiry: DirectInquiryRequest):
    """
    Send direct inquiry email via Resend Email API
    Flow: Form -> FastAPI -> Resend API -> Admin Gmail inbox
    """
    if not RESEND_API_KEY:
        raise ValueError(
            "RESEND_API_KEY is not configured in your .env file. "
            "Please obtain your free API key from https://resend.com/api-keys and set RESEND_API_KEY=re_... in .env"
        )

    # Clean, responsive HTML email template for MediCare Inquiries
...
<head>
  ...
  <style>
    ...
    .header {{ background: linear-gradient(135deg, #0077B6 0%, #00B4D8 100%); color: #ffffff; padding: 26px 30px; text-align: center; }}
    .header h1 {{ margin: 0; font-size: 22px; font-weight: 700; letter-spacing: -0.5px; }}
    .header p {{ margin: 6px 0 0 0; opacity: 0.92; font-size: 13.5px; }}
    .content {{ padding: 28px 30px; }}
    .info-table {{ width: 100%; border-collapse: collapse; margin-bottom: 24px; }}
    .info-table td {{ padding: 11px 12px; border-bottom: 1px solid #f1f5f9; font-size: 14px; }}
    .info-table td.label {{ font-weight: 600; color: #475569; width: 35%; background: #f8fafc; border-radius: 4px 0 0 4px; }}
    .info-table td.value {{ color: #0f172a; }}
    .dept-badge {{ display: inline-block; background: #e0f2fe; color: #0369a1; padding: 3px 9px; border-radius: 6px; font-weight: 600; font-size: 12.5px; }}
    .msg-title {{ font-size: 14px; font-weight: 600; color: #334155; margin: 0 0 8px 0; text-transform: uppercase; letter-spacing: 0.5px; }}
    .msg-box {{ background: #f8fafc; border-left: 4px solid #0077B6; padding: 16px 20px; border-radius: 0 8px 8px 0; font-size: 14.5px; line-height: 1.65; color: #1e293b; white-space: pre-wrap; }}
    .footer {{ background: #f8fafc; border-top: 1px solid #e2e8f0; padding: 16px 30px; text-align: center; font-size: 12px; color: #64748b; }}
  </style>
</head>
<body>
  <div class="container">
    <div class="header">
      <h1>🏥 MediCare Hospital System</h1>
      <p>Direct Inquiry from Website Contact Portal</p>
    </div>
    <div class="content">
      <table class="info-table">
        <tr>
          <td class="label">Sender Name:</td>
          <td class="value"><strong>{inquiry.full_name}</strong></td>
        </tr>
        <tr>
          <td class="label">Email Address:</td>
          <td class="value"><a href="mailto:{inquiry.email}" style="color:#0077B6; text-decoration:none; font-weight:500;">{inquiry.email}</a></td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td class="value"><span class="dept-badge">{inquiry.department}</span></td>
        </tr>
        <tr>
          <td class="label">Received Date:</td>
          <td class="value">{datetime.now().strftime('%d %B %Y, %I:%M %p')}</td>
        </tr>
      </table>

      <div class="msg-title">Inquiry Message:</div>
      <div class="msg-box">{inquiry.message}</div>
    </div>
    <div class="footer">
      <p style="margin: 0;">Delivered directly to Admin Inbox: <strong>{ADMIN_INQUIRY_EMAIL}</strong></p>
      <p style="margin: 4px 0 0 0;">MediCare Hospital Management System © 2026</p>
    </div>
  </div>
</body>
</html>
"""
    ...
    plain_text = f"""New Hospital Direct Inquiry:
------------------------------------------
Sender Name:  {inquiry.full_name}
Email:        {inquiry.email}
Phone:        {inquiry.phone}
...
{inquiry.message}
------------------------------------------
Delivered to: {ADMIN_INQUIRY_EMAIL}
"""
    ...
    payload = {
        ...
        "subject": f"🏥 New Hospital Inquiry: {inquiry.department} - {inquiry.full_name}",
        "html": html_content,
        "text": plain_text
    }
    ...
    async with httpx.AsyncClient(timeout=15.0) as client:
        ...
        if res.status_code not in [200, 201]:
            try:
                ...
                err_msg = res.text
            raise RuntimeError(f"Resend API error ({res.status_code}): {err_msg}")
...
async def submit_direct_inquiry(inquiry: DirectInquiryRequest):
    ...
    Sends email to admin Gmail via Resend Email API and logs to Supabase
    """
    try:
        # 1. Send email via Resend Email API
        await send_inquiry_email(inquiry)

        # 2. Log to Supabase activity logs
        try:
            supabase_admin.table("activity_logs").insert({
        ...
        return {
            "success": True,
            "message": f"Inquiry sent successfully to {ADMIN_INQUIRY_EMAIL}"
        }
