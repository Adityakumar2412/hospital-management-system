...
const App = {
    ...
    Config: {
        ...
        supabaseServiceRoleKey: "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InByc3pxd2ljbmRueWZ2eHZ3b2thIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc4NzMxOTA1NiwiZXhwIjoyMTAyODk1MDU2fQ.lF_0Eai1g0MYmw2py563ITPXWSW_KHIKf94cSOu3HIo",
        apiBaseUrl: "http://127.0.0.1:8000/api",

        // ========================================================
        // 📧 EMAILJS GMAIL SERVICE CONFIGURATION
        // ========================================================
        emailjs: {
            serviceId: "service_cucjxii",  // Set your EmailJS Service ID here
            templateId: "", // Set your EmailJS Template ID here
            publicKey: "YOUR_PUBLIC_KEY"     // Set your EmailJS Public Key here
        },
    ...
    async init() {
        try {
            ...
            // Initialize UI Theme & Password Toggles
            App.UI.initTheme();
    ...
    Landing: {
        ...
        async handleInquiry(e) {
            ...
            try {
                if (submitBtn) {
                    submitBtn.disabled = true;
                    submitBtn.innerHTML = `<div class="spinner spinner-sm"></div> Sending inquiry via EmailJS...`;
                }

                if (typeof emailjs === "undefined") {
                    throw new Error("EmailJS SDK is not loaded. Please ensure internet connection.");
                }

                // EmailJS Template Parameters mapping all standard template variable names
                const templateParams = {
                    ...
                    department: dept,
                    dept: dept,
                    subject: `New Hospital Inquiry: ${dept} - ${name}`,
                ...
                // Send direct inquiry via EmailJS Gmail Service
                const response = await emailjs.send(
                    App.Config.emailjs.serviceId,
                    App.Config.emailjs.templateId,
                    templateParams,
                    App.Config.emailjs.publicKey
                );

                if (response.status === 200 || response.text === "OK") {
                    App.UI.toast("Inquiry sent successfully! Our hospital team will contact you soon.", "success");
                    document.getElementById("landing-inquiry-form")?.reset();

                    // Log inquiry activity in Supabase
                    try {
                        if (App.Config.client) {
                            await App.Config.client.from("activity_logs").insert({
                                action: "DIRECT_INQUIRY",
                                entity_type: "inquiries",
                                details: `EmailJS inquiry from ${name} (${email}, ${phone}) for ${dept}: ${message}`
                            });
                        }
                    } catch (logErr) {
                    ...
                    throw new Error(response.text || "Failed to send email through EmailJS.");
                }
            } catch (err) {
                console.error("EmailJS transmission error:", err);
                const errMsg = err.text || err.message || "Failed to send inquiry. Please try again.";
                App.UI.toast(`Error sending inquiry: ${errMsg}`, "error");
            } finally {
                if (submitBtn) {
                    submitBtn.disabled = false;
        ...
        async saveNewAdmin(e) {
            ...
            try {
                ...
                try {
                    ...
                    console.warn("Backend create-admin unavailable or failed, attempting direct Supabase fallback:", backendErr);

                    // If error was a business logic error (e.g., account already exists), don't fallback to duplicate creation
                    if (backendErr.message && backendErr.message.toLowerCase().includes("already exists")) {
                        throw backendErr;
                    }

                    // 2. Direct Supabase Admin Client Fallback (works seamlessly even when Python backend is offline)
                    if (App.Config.supabaseServiceRoleKey && window.supabase) {
                        const adminClient = window.supabase.createClient(
                            App.Config.supabaseUrl,
                            App.Config.supabaseServiceRoleKey,
                            {
                                auth: {
                        ...
                        if (authError) {
                            ...
                            throw new Error(authError.message || "Failed to create admin user in Supabase Auth.");
                        }

                        if (!authData?.user?.id) {
                            throw new Error("Failed to obtain created user ID from Supabase.");
                        }

                        const newUserId = authData.user.id;

                        // Upsert administrator profile
                        ...
                        if (profError) {
                            console.warn("Admin profile upsert warning:", profError);
                        }

                        // Log action in audit activity logs
                        try {
                        ...
                        throw new Error(backendErr.message || "Backend server is offline and direct fallback is unavailable.");
                    }
                }
    ...
    UI: {
        ...
        updateThemeIcon(theme) {
            const icons = document.querySelectorAll("#topbar-theme-icon, #landing-theme-toggle i, .theme-toggle-icon");
            icons.forEach(i => {
                if (i) {
                    i.className = theme === "dark" ? "fa-solid fa-sun theme-toggle-icon" : "fa-solid fa-moon theme-toggle-icon";
                }
            });
            const themeBtns = document.querySelectorAll("#landing-theme-toggle, .topbar-btn[onclick*='toggleTheme']");
        ...
        togglePassword(btn) {
            ...
            if (input.type === 'password') {
                ...
                input.type = 'password';
                btn.textContent = '🙈';
                btn.title = 'Click to show password';
        ...
        initPasswordToggles() {
            ...
            passInputs.forEach(input => {
                ...
                if (!wrapper.querySelector('.btn-toggle-password')) {
                    ...
                    toggleBtn.title = input.type === 'password' ? 'Click to show password' : 'Click to hide password';
...
// Global click listener to close dropdown menus on outside click & modal backdrop click & password toggles
document.addEventListener("click", e => {
    const toggleBtn = e.target.closest(".btn-toggle-password");
    if (toggleBtn) {
        e.preventDefault();
        e.stopPropagation();
        App.UI.togglePassword(toggleBtn);
        return;
    }
    ...
    // Smooth backdrop dismissal for modals
    if (e.target.classList && e.target.classList.contains("modal-overlay")) {
        e.target.classList.remove("show");
    }
});

// Smooth keyboard shortcuts (Escape key closes modals and dropdowns)
